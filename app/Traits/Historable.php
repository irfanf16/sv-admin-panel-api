...
trait Historable
{
    ...
    public static function bootHistorable()
    {
        ...
        static::created(function (Model $model) {
            $fill = $model->toArray();
            if(isset($fill['password']) || isset($fill['app_password'])) {
                $fill['password'] = 'some password';
                $fill['app_password'] = 'some password';
            }
            $model->writeHistory('created', Str::limit($model->present()->title, 200, '…'), [], $fill);
        });
        ...
        static::updated(function (Model $model) {
            ...
            if(isset($old['password']) || isset($old['app_password'])) {
                $old['password'] = 'some password';
                $old['app_password'] = 'some password';
            }
            if(isset($new['password']) || isset($new['app_password'])) {
                $new['password'] = 'some password';
                $new['app_password'] = 'some password';
            }
